<x-app-layout>
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('teams.update', $team->id) }}" method="POST">
            ...
            <div class="mb-3">
                <label>Stad</label>
                <input type="text" name="city" class="form-control" value="{{ old('city', $team->city) }}" required>
            </div>

            <div class="mb-3">
                <label>Coach naam</label>
                <input type="text" name="coach_name" class="form-control" value="{{ old('coach_name', $team->coach_name) }}" required>
            </div>
            ...
            <div id="players-wrapper" class="mb-3">
                ...
                @foreach($team->players as $player)
                    <div class="player-row mb-2">
                        <input type="text" name="players[]" class="form-control mb-1" value="{{ $player->name }}" required>
                    </div>
                @endforeach
            </div>
            ...
            <div>
                ...
                <a href="{{ route('home') }}" class="btn btn-primary">Terug naar homepage</a>
            </div>
        </form>
    </div>
